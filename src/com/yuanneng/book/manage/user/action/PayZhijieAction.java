...
public class PayZhijieAction extends CommonAction {
    ...
    public String doMain() throws Exception {
        ...
        @SuppressWarnings("unchecked")
        List<PayBean> payBeans = (List<PayBean>) session.getAttribute("ZhifuList");
        if(payBeans == null || payBeans.size()==0) {
            payBeans = (List<PayBean>) session.getAttribute("ZhifuList_pay");
        }
        userService.pay_zhijie(payBeans,select,paySum,payName);
