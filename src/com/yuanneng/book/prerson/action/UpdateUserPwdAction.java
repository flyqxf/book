...
import com.yuanneng.book.prerson.service.UpdatePwdService;
 ...
 * <pre>
 * ver     修订日             作者            修订内容
 * 1.0     2016.03.31         shanjixiang        新规做成
 * </pre>
 */
public class UpdateUserPwdAction extends CommonAction {
    ...
    public String doMain() throws Exception {
        ...
        /** 取得UVO */
        UserValueObject uvo = (UserValueObject) this.getSession().getAttribute(Constant.UVO);

        /** 取得旧密码向数据库查询确认 */
        UserBean rst = updatePwdService.equelUserPwd(uvo.getUser().getUserId(), nowPwd);
        if (null == rst) {
            msg = messageHelper
        ...
        condition.setUserId(uvo.getUser().getUserId());
        int result = updatePwdService.changePwd(condition);
        /** 修改密码失败 */
