...
var addRow=function(){
        ...
        $("#settingTable").append(
                ...
                '<td><s:textfield id="title'+totalIndex+'" name="successList['+ totalIndex + '].title"  class="scinput" theme="simple"/></td>'+
                '<td><s:textfield id="createUser'+totalIndex+'" name="successList['+ totalIndex + '].createUser" class="scinput" theme="simple"/></td>'+
                '<td><s:textfield id="link'+totalIndex+'" name="successList['+ totalIndex + '].link" class="scinput" theme="simple" maxlength="50"/></td>'+
...
var removeRow=function(rownum) {
    ...
    for(var i = rownum;i<totalIndex;i++) {
        ...
        if(idx != lastHidIdx) {
            ...
            $("#title"+idx).val($("#title"+j).val());
            $("#createUser"+idx).val($("#createUser"+j).val());
...
<form id="jiaoxueForm" method="post" action="../successManage/updateSuccess.action">
  
<table id="settingTable" width="700"  border="3" class="tablelist">
  <tr>
    <th width="240" bgcolor="#FF9900" scope="col">标题</th>
    <th width="120" bgcolor="#FF9900" scope="col">用户</th>
    <th width="120" bgcolor="#FF9900" scope="col">内容链接</th>
    <th width="200"bgcolor="#FF9900" scope="col">操作</th>
  </tr>
  <s:iterator id="bean" status="result"  value="successManagerBeans">
  <tr id="row<s:property value="#result.index" />">
    <td><s:textfield id="title%{#result.index}" name="successList[%{#result.index}].title" value="%{title}" class="scinput" theme="simple"/></td>
    <td><s:textfield id="createUser%{#result.index}" name="successList[%{#result.index}].createUser" value="%{createUser}" class="scinput" theme="simple"/></td>
    <td><s:textfield id="link%{#result.index}" name="successList[%{#result.index}].link" value="%{link}" class="scinput" theme="simple" maxlength='50'/></td>
